...
document.getElemetsByClassName = function (name, target) {
  ...
  if (!object)
    object = document.getElementsByTagName
...
PageSize.prototype.update = function () {
  ...
  var w =
    window.innerWidth && window.scrollMaxX
      ? window.innerWidth + window.scrollMaxX
      : d.body.scrollWidth > d.body.offsetWidth
        ? d.body.scrollWidth
        : d.body.offsetWidth;
  var h =
...
if (!window.Spica) {
  ...
  Spica.Browser = function () {
    ...
    this.isSafari3 =
      this.name.match(/AppleWebKit\/(\d\d\d)/) && parseInt(RegExp.$1) > 500;
    this.isKHTML =
      this.isSafari || navigator.appVersion.match(/Konqueror|KHTML/);
  ...
  Spica.Event = {
    ...
    register: function (object, type, handler) {
      ...
      if (type == "mousewheel" && Spica.Browser.isGecko)
        type = "DOMMouseScroll";
...
Lightbox.prototype = {
  ...
  _set_photo_size: function () {
    ...
    if (caption) heightmargin += caption.clientHeight || caption.offsetHeight;
    ...
    if (!self._expanded) {
      ...
      if ((orig.w >= targ.w || orig.h >= targ.h) && orig.h && orig.w)
        ratio =
          targ.w / orig.w < targ.h / orig.h ? targ.w / orig.w : targ.h / orig.h;
  ...
  _show_overall: function (visible) {
    ...
    if (visible) {
      ...
      inner.x =
        outer.x - parseInt(((self._imgpos.x - self._pos.x) / max) * base);
      inner.y =
        outer.y - parseInt(((self._imgpos.y - self._pos.y) / max) * base);
  ...
  _reset_func: function () {
    ...
    if (self._funcs.wheel !== null)
      Spica.Event.deregister(self._box, "mousewheel", self._funcs.wheel);
    if (self._funcs.move !== null)
      Spica.Event.deregister(self._img, "mousemove", self._funcs.move);
    if (self._funcs.up !== null)
      Spica.Event.deregister(self._img, "mouseup", self._funcs.up);
    if (self._funcs.drag !== null)
      Spica.Event.deregister(self._img, "mousedown", self._funcs.drag);
    if (self._funcs.dbl !== null)
      Spica.Event.deregister(self._img, "dblclick", self._funcs.dbl);
    self._funcs = { move: null, up: null, drag: null, wheel: null, dbl: null };
  },
  ...
  _toggle_wrap: function (flag) {
    ...
    if (self._hiding.length === 0 && !self._first) {
      ...
      for (i = 0, n = tags.length; i < n; i++) {
        ...
        for (var j = 0, m = elem.length; j < m; j++) {
          ...
          if (!check) {
            if (elem[j].currentStyle) check = elem[j].currentStyle.visibility;
            else if (document.defaultView)
              check = document.defaultView
                .getComputedStyle(elem[j], "")
    ...
    if (flag) self._setOpacity(self._wrap, 5);
  },
  _prepare: function () {
    var self = this;
    if (self._open == -1) return;
  ...
  _show: function (num) {
    ...
    imag.onload = function () {
      ...
      if (!self._anim.f)
        // animator is disabled, so apply immediately
        self._show_image();
  ...
  _run: function () {
    ...
    if (
      self._anim.step === 0 ||
      self._anim.w != self._img.width ||
      self._anim.h != self._img.height
    ) {
      self._doResizing();
    }
  ...
  _show_image: function () {
    ...
    if (
      effect &&
      (!effect.className || self._imgs[self._open].cls == effect.className)
    )
  ...
  _doResizing: function () {
    ...
    var done =
      self._anim.w == self._img.width && self._anim.h == self._img.height;
  ...
  _setOpacity: function (elem, alpha) {
    ...
    else elem.style.opacity = alpha / 10;
  },
  _close_box: function () {
    var self = this;
    self._open = -1;
